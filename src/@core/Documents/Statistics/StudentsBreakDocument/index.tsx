...
interface Prop {
  datasource: StudentsBreaksDto;
}

const StudentsBreakDocument = ({ datasource }: Prop) => {
  return (
    <Document>
      <Page size='A4' style={styles.page}>
        ...
        <View style={styles.section}>
          <Text style={styles.documentTitle}>
            جدول أوقات فراغ التلاميذ للموسم الدراسي  {datasource.yearTitle}
          </Text>
        </View>
        ...
        <View style={tableStyles.table}>
          <View style={tableStyles.tableRow}>
            ...
            >
              <Text style={tableStyles.tableCellHeader}>الفوج</Text>
            </View>
          </View>
          {datasource.classBreakDays &&
            datasource.classBreakDays?.map((row, index) => (
              <View style={tableStyles.tableRow}>
                <View style={[tableStyles.tableCol, { flexBasis: '23.33%' }]}>
                  <Text style={tableStyles.tableCellHeader}>{row.thursday}</Text>
                </View>
                <View style={[tableStyles.tableCol, { flexBasis: '23.33%' }]}>
                  <Text style={tableStyles.tableCellHeader}>{row.wednesday}</Text>
                </View>
                <View style={[tableStyles.tableCol, { flexBasis: '23.33%' }]}>
                  <Text style={tableStyles.tableCellHeader}> {row.tuesday} </Text>
                </View>
                <View style={[tableStyles.tableCol, { flexBasis: '23.33%' }]}>
                  <Text style={tableStyles.tableCellHeader}> {row.monday} </Text>
                </View>
                <View style={[tableStyles.tableCol, { flexBasis: '23.33%' }]}>
...
export default StudentsBreakDocument;
