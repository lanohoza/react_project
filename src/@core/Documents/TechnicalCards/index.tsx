...
interface Prop {
  datasources: TechnicalCardDocumentDto[];
}

const TechnicalCardDocuments = ({ datasources }: Prop) => {
      ...
      {datasources.map(datasource => (
        ...
        <Page size='A4' style={styles.page}>
          ...
          <View style={[styles.section, { flexDirection: "row", justifyContent: "center" }]}>
            ...
            <Text style={[styles.documentTitle, { fontSize: 14, border: "1px solid black", borderRadius: "10%", padding: 5, width: 150, }]}>
          ...
          <View style={[styles.section, , { flexDirection: "row", justifyContent: "flex-end" }]}>
            ...
            <Text style={[styles.text, { fontWeight: "bold", fontSize: 10 }]}>: رمز البطاقة -</Text>
          </View>
          <View style={[styles.section, , { flexDirection: "row", justifyContent: "flex-end" }]}>
            <Text style={[styles.text]}>{datasource.title}</Text>
            <Text style={[styles.text, { fontWeight: "bold", fontSize: 10 }]}>: عنوان النشاط -</Text>
          </View>
          <View style={[styles.section, , { flexDirection: "row", justifyContent: "flex-end" }]}>
            <Text style={[styles.text]}>{datasource.type}</Text>
            <Text style={[styles.text, { fontWeight: "bold", fontSize: 10 }]}>: نوع النشاط -</Text>
          </View>
          <View style={[styles.section, , { flexDirection: "row", justifyContent: "flex-end" }]}>
            <Text style={[styles.text]}>{datasource.category}</Text>
            <Text style={[styles.text, { fontWeight: "bold", fontSize: 10 }]}>: باب النشاط -</Text>
          </View>
          <View style={[styles.section, , { flexDirection: "row", justifyContent: "flex-end" }]}>
            ...
            <Text style={[styles.text, { fontWeight: "bold", fontSize: 10 }]}>: الجمهور المستهف -</Text>
          </View>
          <View style={[styles.section, , { flexDirection: "row", justifyContent: "flex-end" }]}>
            <Text style={[styles.text, { fontWeight: "bold", fontSize: 10 }]}>:  الاهداف العامة -</Text>
          </View>
          {datasource?.generalObjectsTitles?.map((generalObjectsTitle, index) => (
            <Text style={[styles.text, { fontSize: 9, marginRight: 100 }]}>{generalObjectsTitle}-</Text>
          ))}
          <View style={[styles.section, , { flexDirection: "row", justifyContent: "flex-end" }]}>
            <Text style={[styles.text, { fontWeight: "bold", fontSize: 10 }]}>:  الاهداف التنفيذية -</Text>
          </View>
          {datasource?.operateObjectsTitles?.map((operateObjectsTitle, index) => (
            <View style={[styles.section, { flexDirection: "row", justifyContent: "flex-end" }]}>
              <Text style={[styles.text, { fontSize: 9, marginRight: 90 }]}>{operateObjectsTitle}-</Text>
            </View>))}
          <View style={[styles.section, , { flexDirection: "row", justifyContent: "flex-end" }]}>
            <Text style={[styles.text]}>{datasource.materielToots}</Text>
            <Text style={[styles.text, { fontWeight: "bold", fontSize: 10 }]}>:   الوسائل المادية -</Text>
          </View>
          <View style={[styles.section, , { flexDirection: "row", justifyContent: "flex-end" }]}>
            ...
            ))}          <Text style={[styles.text, { fontWeight: "bold", fontSize: 10 }]}>:  الوسائل البشرية  -</Text>
          </View>
          <View style={[styles.section, , { flexDirection: "row", justifyContent: "flex-end" }]}>
            <Text style={[styles.text, { fontWeight: "bold", fontSize: 10 }]}>:  الصعوبات المستهدفة بالنشاط  -</Text>
          </View>
          {datasource?.difficulties?.map((difficulty, index) => (
            <View style={[styles.section, { flexDirection: "row", justifyContent: "flex-end" }]}>
              <Text style={[styles.text, { fontSize: 9, marginRight: 90 }]}>{difficulty}-</Text>
            </View>))}
          <View style={[styles.section, , { flexDirection: "row", justifyContent: "flex-end" }]}>
            <Text style={[styles.text]}>{datasource.monthTitle}</Text>
            <Text style={[styles.text, { fontWeight: "bold", fontSize: 10 }]}>:  شهر الانجاز -</Text>
          </View>
          <View style={[styles.section, , { flexDirection: "row", justifyContent: "flex-end" }]}>
            <Text style={[styles.text]}>{datasource.weekTitle}</Text>
            <Text style={[styles.text, { fontWeight: "bold", fontSize: 10 }]}>: أسبوع الانجاز -</Text>
          </View>
          <View style={[styles.section, , { flexDirection: "row", justifyContent: "flex-end" }]}>
            <Text style={[styles.text, { fontWeight: "bold", fontSize: 10 }]}>: النصوص الرسمية -</Text>
          </View>
          {datasource?.officialTxts?.map((officialTxt) => (
            <View style={[styles.section, , { flexDirection: "row", justifyContent: "flex-end" }]}>
              <Text style={[styles.text, { fontSize: 9, marginRight: 90 }]}>{officialTxt.title}-</Text>

            </View>))}
          <View style={[styles.section, , { flexDirection: "row", justifyContent: "flex-end" }]}>
            <Text style={[styles.text]}>{datasource.feedback}</Text>
            <Text style={[styles.text, { fontWeight: "bold", fontSize: 10 }]}>: مؤشر التحقق -</Text>
          </View>
...
export default TechnicalCardDocuments;
