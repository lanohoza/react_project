...
import {
  getEstablishmentCardData,
  getLevelCardData,
  getStudentCardData,
} from '@core/services/CardService';
...
const EstablishmentCard = () => {
  ...
  const loadLevelData = () => {
    if (idYear != -1) {
      getEstablishmentCardData(idYear, infoViewActionsContext).then(
        (establishment) => {
          setCardSource(establishment);
        },
      );
    }
  };
  ...
  return (
    <StyledProductDetails>
      <AppsContainer title='بطاقة المؤسسة ' fullView>
        <AppPageMeta title='بطاقة المؤسسة ' />
        <AppAnimate animation='transition.slideUpIn' delay={200}>
          <AppCard key='product_detail'>
            ...
            {cardSource?.length > 0 && (
              <TCE002Card source={cardSource}></TCE002Card>
            )}
         
            {cardSource?.length > 0 && (
              <TCO001Card source={cardSource}></TCO001Card>
            )}
          </AppCard>
        </AppAnimate>
      </AppsContainer>
