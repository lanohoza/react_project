...
import { TCO0001EstablishmentCardDto } from '@core/types/models/script/TCO001Script';
...
const TCO001Card = ({ source }: Prop) => {
  ...
  const [data, setData] = useState<TCO0001EstablishmentCardDto>(undefined);
              ...
              <table width={'100%'} border={1} style={{ textAlign: 'center' }}>
                <thead>
                  <tr>
                    ...
                    <th rowSpan={3}>معدل المادة الاساسية الأولى</th>
                    <th rowSpan={3}>معدل المادة الاساسية الثانية</th>
                ...
                <tbody>
                  {data.guidanceSpecialityAverage.map((sAverage) => (
                    <tr>
                      ...
                      <td>{sAverage.subjectAverage1.toFixed(2)}</td>
                      <td>{sAverage.subjectAverage2.toFixed(2)}</td>
