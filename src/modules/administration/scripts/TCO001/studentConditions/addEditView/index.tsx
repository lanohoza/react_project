...
import { getSubjectsByType } from '@core/services/SubjectService';
import {
...
} from '../TCO002StudentConditionsContextProvider';
import {
  createTCO002StudentConditions,
  updateTCO002StudentConditions,
...
const AddEditViewTCE002Condition: React.FC = () => {
  ...
  const [selectedDiagnosticType, setSelectedDiagnosticType] =
    useState<DiagnosticType | null>(DiagnosticType.student);
  ...
  const onFinish = async (values: any) => {
    ...
    if (modeAddEditViewModel == ModeComponent.create) {
      createTCO002StudentConditions(dto, infoViewActionsContext)
        .then(() => {
      ...
      updateTCO002StudentConditions(dto.id, dto, infoViewActionsContext)
        .then(() => {
          infoViewActionsContext.showMessage('تم الحفظ بنجاح');
          reload();
          onCloseEditViewModel();
        ...
        <StyledFormHeader>
          <StyledFormHeaderTitle>
            {modeAddEditViewModel === ModeComponent.create
              ? 'إنشاء شروط التشخيص التلميذ'
              : modeAddEditViewModel === ModeComponent.edit
                ? 'تعديل شروط التشخيص التلميذ'
                : 'عرض شروط التشخيص التلميذ'}
          </StyledFormHeaderTitle>
        </StyledFormHeader>
        <StyledFormContent>
          <StyledFormContentItem>
