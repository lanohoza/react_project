...
import Auth, { confirmSignUp, getCurrentUser } from '@aws-amplify/auth';
...
const AwsAuthProvider: React.FC<AwsAuthProviderProps> = ({ children }) => {
  ...
  const confirmCognitoUserSignup = async (
    username: string,
  ...
  const forgotPassword = async (username: string, confirmationCode: string) => {
    infoViewActionsContext.fetchStart();
    try {
